...
Host agent for Riko Agent. Exposes a minimal HTTP API to allow an agent control
client to request screenshots and execute actions. This is the canonical agent
version; legacy copies were previously under `remote_setup/`.
...
- POST /update -> force immediate update check; requires auth

Security: use `AGENT_API_TOKEN` as the primary token; `REMOTE_API_TOKEN` is no
longer required and kept only for one-step migration support.
"""
...
Simple host/agent script to run on the machine. Exposes a minimal HTTP API to
allow clients to request screenshots and execute actions.

This version uses AGENT_API_TOKEN environment variable and writes audit logs to agent_audit.jsonl
"""
